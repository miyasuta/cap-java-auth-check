...
| 6 | `@requires` を action/function に直接置いたパターン（対比） | **CAP Java では機能しない**。`Admin6` ロールを持つ alice も 403 になる。エンティティ側の `@restrict` で `grant` に action/function 名を指定する方法のみサポート | ⚠️ 想定と異なる |
...
- [検証結果（詳細）](#検証結果)
  ...
  - [Topic 6: @requires を action/function に直接置いたパターン](#topic-6-requires-を-actionfunction-に直接置いたパターン対比)
...
service OrderService @(requires: 'authenticated-user') {
  ...
  // エンティティ自体は @restrict で READ/CREATE/DELETE を許可
  // action/function には直接 @requires を付与 → CAP Java では機能しない
  @(restrict: [
    { grant: 'READ' },
  ...
  actions {
    @(requires: 'Admin6')
    action directAction();      // ← CAP Java では @requires が効かない
    @(requires: 'Admin6')
...
### Topic 6: `@requires` を action/function に直接置いたパターン（対比）

**CAP Java 4.8.0 では、action/function に直接置いた `@requires` は機能しない。**

アノテーション構成:
- エンティティ: `@restrict` で `READ` と `CREATE/DELETE` を `Admin6` に許可
- action/function: `@requires: 'Admin6'` を直接付与

| テスト | ユーザー | 操作 | 結果 | 意味 |
|--------|---------|------|------|------|
| 6-A | alice（Admin6ロール） | JP Order の `directAction` | **403** ⚠️ | `@requires` が機能していない |
| 6-B | viewer（ロールなし） | JP Order の `directAction` | **403** | alice と区別がつかない |
| 6-C | alice（Admin6ロール） | JP Order の `directFunction` | **403** ⚠️ | function も同様 |
| 6-D | viewer（ロールなし） | JP Order の `directFunction` | **403** | alice と区別がつかない |

**考察**:
- CSN（コンパイル済みモデル）には `@requires: 'Admin6'` が正しく付与されている
- しかし CAP Java ランタイムは action/function に直接付いた `@requires` を評価しない
- CDS ドキュメントの表の脚注（action/function の `where` に関する記述）は Node.js の話として書かれており、**Java における action/function 直接アノテーションの動作は明記されていない**
- **結論: CAP Java では action/function の権限制御はエンティティ側の `@restrict` で `grant` に action/function 名を指定する方法のみサポートされる**
...
| 6 | action/function への直接アノテーション | **CAP Java では機能しない**。エンティティ側の `@restrict` で `grant` に指定する方法のみ有効 |
